...
import { todoService } from '../services.js/TodoServices'
...
useEffect(()=>{
    todoService.get_lists()
    .then(response => response.json())
    .then(data => {setLists(data.data); console.log(lists)})
    .catch(err => alert(err))
}, [renderFix])

function handleDelete(list){
    console.log(list._id)
    todoService.delete_list(list._id)
    .then(response => response.json())
    .then(data => {
        if (data.success){
            alert("deleted")
                    ...
                    lists.map((list, index) =>
                        <tr key={list._id}>
                            ...
                            <td>
                                ...
                                <span onClick={()=>{handleDelete(list)}}>
                                    <svg
                                                width="15"
                                                height="15"
                                                viewBox="0 0 16 18"
